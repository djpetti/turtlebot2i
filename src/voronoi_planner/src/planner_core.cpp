...
/// Minimum length we allow for partial paths.
constexpr double kMinPartialPathLength = 1.0;
/**
 * @brief Maximum value for a cell that we allow before we consider it blocked.
 */
constexpr double kMaxFreeCostmapValue = costmap_2d::LETHAL_OBSTACLE;
 ...
 */
double EuclideanDistance(const std::pair<float, float> &point1,
                         const std::pair<float, float> &point2) {
  return std::sqrt(std::pow(std::get<0>(point2) - std::get<0>(point1), 2) +
 ...
 */
double FindPathLength(const std::vector<std::pair<float, float>> &path) {
  if (path.empty()) {
    // Zero-length path.
    return 0.0;
  }
  ...
  auto previous_point = path[0];
  for (const auto &point : path) {
    total_distance += EuclideanDistance(previous_point, point);
...
VoronoiPlanner::VoronoiPlanner(std::string name, costmap_2d::Costmap2D *costmap,
                               std::string frame_id)
    : costmap_(NULL), initialized_(false), publish_voronoi_grid_(true),
      smooth_path_(true), weight_data_(0.5), weight_smooth_(0.3) {
...
void VoronoiPlanner::initialize(std::string name,
...
void VoronoiPlanner::initialize(std::string name,
  ...
  if (!initialized_) {
    ...
    make_plan_srv_ = private_nh.advertiseService(
        "make_plan", &VoronoiPlanner::makePlanService, this);

    dsrv_ =
        new dynamic_reconfigure::Server<voronoi_planner::VoronoiPlannerConfig>(
            ros::NodeHandle("~/" + name));
    dynamic_reconfigure::Server<
        voronoi_planner::VoronoiPlannerConfig>::CallbackType cb =
        boost::bind(&VoronoiPlanner::reconfigureCB, this, _1, _2);
    dsrv_->setCallback(cb);

    ros::Subscriber costmapUpdateSubscriber =
        private_nh.subscribe("/move_base/global_costmap/costmap_updates", 10,
                             &VoronoiPlanner::costmapUpdateCallback, this);
...
void VoronoiPlanner::costmapUpdateCallback(
  ...
  ROS_INFO("Map update. x %d; y %d; w %d; h %d", msg->x, msg->y, msg->width,
           msg->height);
}

void VoronoiPlanner::reconfigureCB(
...
void VoronoiPlanner::clearRobotCell(const tf::Stamped<tf::Pose> &global_pose,
...
bool VoronoiPlanner::makePlanService(nav_msgs::GetPlan::Request &req,
...
bool VoronoiPlanner::makePlan(const geometry_msgs::PoseStamped &start,
...
bool VoronoiPlanner::makePlan(const geometry_msgs::PoseStamped &start,
  ...
  //    ROS_INFO("Time (for visualize): %f sec", (ros::Time::now() -
  //    t).toSec());
  ...
  bool found_path_to_start = false, found_voronoi_path = false,
       found_path_to_goal = false;

  if (!voronoi_.isVoronoi(goal_x, goal_y)) {
    //        path3 = findPath( goal, init, A, 0, 1 );
    found_path_to_goal =
        findPath(&path3, goal_x, goal_y, start_x, start_y, &voronoi_, 0, 1);
    std::cout << "findPath 3 res " << found_path_to_goal << std::endl;
    //        goal = path3(end,:);
    if (!path3.empty()) {
  ...
  if (!voronoi_.isVoronoi(start_x, start_y)) {
    found_path_to_start =
        findPath(&path1, start_x, start_y, goal_x, goal_y, &voronoi_, 0, 1);
    std::cout << "findPath 1 res " << found_path_to_start << std::endl;
    if (!path1.empty()) {
  ...
  found_voronoi_path =
      findPath(&path2, start_x, start_y, goal_x, goal_y, &voronoi_, 1, 0);
  std::cout << "findPath 2 res " << found_voronoi_path << std::endl;
  ...
  const bool found_full_path =
      found_path_to_start && found_voronoi_path && found_path_to_goal;
  if (!found_full_path) {
    ROS_WARN("Failed to find full path");
  }

  // If we didn't find a path to the start, this path is automatically invalid.
  bool is_path_valid = found_path_to_start;
  if (!found_full_path) {
    // Determine whether the path is valid. For our purposes, we characterize
    // a very short path as possibly invalid. If we can only compute a tiny
    // path, it's probably better to let it run the recovery behaviors in the
    // hope of getting a better map, instead of just sitting there.
    const double path_length = FindPathLength(path1);
    ROS_INFO_STREAM("Found partial path with a length of " << path_length
                                                           << ".");

    if (path_length < kMinPartialPathLength) {
      ROS_WARN_STREAM("Partial path of length " << path_length
                                                << " is too short.");
      is_path_valid = false;
    }
  }
  ...
  for (int i = 0; i < path1.size(); i++) {
    ...
    // For now, set all orientations along the path to the goal orientation.
    // TODO (daniel) Do this more intelligently.
...
bool VoronoiPlanner::findPath(std::vector<std::pair<float, float>> *path,
  ...
  //    ROS_INFO("init_x %d, init_y %d, goal_x %d, goal_y %d,
  //    check_is_voronoi_cell %d, stop_at_voronoi %d", init_x, init_y, goal_x,
  //    goal_y, check_is_voronoi_cell, stop_at_voronoi);
  //    ROS_INFO("isVoronoi(init) %d; isVoronoi(goal) %d",
  //    voronoi->isVoronoi(init_x, init_y), voronoi->isVoronoi(goal_x, goal_y)
  //    );
...
void VoronoiPlanner::publishPlan(
